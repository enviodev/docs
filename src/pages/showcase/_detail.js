...
// A hard slice at 160 chars cut 13 of the 18 entries mid-word, so search
// results showed things like "it provides real-time ind". Trim back to the
// last whole word instead, and drop any backticks, which are markdown to us
// but literal characters inside a meta tag.
function metaDescriptionFor(site) {
  const source = (site.longDescription || site.description || "").replace(
    /`/g,
    ""
  );
...
export default function ShowcaseDetail({ site }) {
  ...
  const metaDescription = metaDescriptionFor(site);

  // Static social card generated by scripts/generate-showcase-og-images.js from
  // this entry's own showcase asset, so a shared link unfurls as the project
  // itself. Absolute, because several platforms drop a relative og:image.
  const ogImage = `${siteConfig.url}${siteConfig.baseUrl}img/showcase/og/${site.slug}.png`;
