...
The Greeter contract is a very simple smart contract that allows a user to write a greeting message on the blockchain and is deployed on both the Polygon and the [Linea](https://linea.xyz/) blockchain, respectively. For this example, we will only look at the `NewGreeting` event.
