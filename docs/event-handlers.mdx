...
Handler functions are used to modify the entities which have been loaded by the `<EventName>.loader` function, and thus should contain all the required logic for updating entities from the raw data emitted by the event.
