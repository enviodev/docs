...
Handler functions are used to modify the entities which have been loaded by the `<event>LoadEntities` function, and thus should contain all the required logic for using raw event information updating the entities.

All of the raw parameters emmited in each event via `event.params`.
