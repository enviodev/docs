...
Loader functions are called via `<EventName>.loader`.
...
Handler functions are called via `<EventName>.handler`.

Handler functions are used to modify the entities which have been loaded by the `<EventName>.loader` function, and thus should contain all the required logic for updating entities with the raw data emitted by the event.

All of the parameters emitted in each event are acceesible via `event.params.<parameterName>`.

Additional raw event information can also be accessed via `event.<parameterName>`.
...
```typescript
import { GreeterContract_registerNewGreetingHandler } from "../generated/src/Handlers.gen";
