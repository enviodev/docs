...
The library is designed to allow developers to mimic the database and mock events and assert different outcomes. The mock database is immutable and each action returns a new instance of a mocked database, this makes it robust and easy to test against previous states of the database.
...
Run codegen when changes are made to the config or schema to regenerate the code helpers
```bash
envio codegen
```
...
#### > set
The ability to set an entity of a MockDb (returns a new MockDb) 
...
```javascript 
updatedMockDb.entities.EntityName.get(id)
```
...
```javascript
ContractName.EventName.createMockEvent({params})
```
...
```javascript 
ContractName.EventName.processEvent({event: mockNewGreetingEvent, mockDb: mockDbInitial})
```
