...
## Learn by doing
Tests are written in JavaScript, TyesScript and ReScript for the Greeter template and can be explored by following the steps below
...
```javascript
const mockEvent = ContractName.EventName.createMockEvent({params})
```
...
{/* Add note sectino about adding in mockEventData to paraas for other event related details */}

#### > `processEvent`
The ability to process an event on a mockDb 

```javascript 
const updatedMockDbFromEvent = ContractName.EventName.processEvent({event: mockEvent, mockDb: updatedMockDb})
```
...
```javascript
 it("A NewGreeting event creates a Greeting entity", () => {
    ...
    const actualGeetingEntity = updatedMockDb.entities.Greeting.get(userAddress);

    // Asserting that the entity in the mock database is the same as the expected entity
    assert.deepEqual(expectedGreetingEntity, actualGeetingEntity);
  });
```
...
```javascript
  it("2 Greetings from the same users results in that user having a greeter count of 2", () => {
    ...
    // Getting the entity from the mock database
    const actualGeetingEntity = updatedMockDb2.entities.Greeting.get(userAddress);

    // Asserting that the field value of the entity in the mock database is the same as the expected field value
    assert.equal(2, actualGeetingEntity?.numberOfGreetings);
  });
```
...
it("2 Greetings from the same users results in that user having a greeter count of 2", () => {
    ...
    // Asserting that the field value of the entity in the mock database is the same as the expected field value
    Assert.equal(actualGeetingEntity.numberOfGreetings, expectedGreetingCount)
  })
```
