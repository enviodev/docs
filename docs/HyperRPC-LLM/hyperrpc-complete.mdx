...
title: HyperRPC Complete Documentation
sidebar_label: HyperRPC Complete Documentation
...
- **What it is:** An extremely fast read only JSON RPC endpoint built on HyperSync and a drop in replacement for traditional RPC nodes

- **Performance:** Up to 5x faster than traditional nodes such as geth, erigon, and reth for data intensive operations

- **Read only:** Optimised for data retrieval only and cannot send transactions

- **Supported networks:** 85+ EVM compatible networks with new networks added regularly

- **Endpoint format:** https://{network}.rpc.hypersync.xyz or https://{chainid}.rpc.hypersync.xyz

- **Supported methods:** eth_chainId, eth_blockNumber, eth_getBlockByNumber, eth_getBlockByHash, eth_getBlockReceipts, eth_getTransactionByHash and more

- **API token:** Recommended, rate limiting starts June 2025

- **When to use:** Drop in replacement for existing RPC based code when tools expect standard JSON RPC interfaces

- **When to use HyperSync instead:** When performance is critical or when you need advanced filtering and field selection

- **Development status:** Under active development and read only
