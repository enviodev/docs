...
Handlers run in registration order. Read more in Multiple Handlers for One Event.
...
Read more in Per-Entity ClickHouse Tuning.
...
Read more in Numeric Entity IDs.
...
- Since v3.5, `@index` is an optimization rather than a requirement: a `getWhere` query on an unindexed field creates the index on demand. Declaring it up front is still faster, because declared indices are built in one batched pass at the end of the backfill. See Deferred Index Creation.
...
Starting from [`v3.4.0`](https://github.com/enviodev/hyperindex/releases/tag/v3.4.0), you can register as many handlers as you like for the same event, each with its own `where` filter. Earlier versions only accepted handlers whose filters matched.
...
// ...and run extra logic for USDC only
indexer.onEvent(
  {
    contract: "ERC20",
    event: "Transfer",
...
Handlers run in the order they were registered. Because registration order is file order, keep handlers for the same event in a predictable place if one depends on the other having run.

:::note
Each registration is fetched and filtered independently, so an event matching two registrations is delivered to both.
:::
...
- Works with any entity field. Since v3.5, if the field has no supporting database index, HyperIndex creates one the first time a handler queries it — you no longer need to declare `@index` up front. Declaring it is still worthwhile for fields you know you'll query: indexes named in the schema are built in one pass at the end of the backfill, whereas an on-demand build pauses writes to that table while it runs. See Deferred Index Creation.
...
Watch `envio_indexing_addresses` in the metrics endpoint to see how many addresses a chain has registered.
...
If you want to index both `Mint` and `Burn` events you can provide multiple filters as an array — an event matches if **any** entry in the array matches (`OR`), while the parameters within a single entry must all match (`AND`). Also, every parameter can accept an array to filter by multiple possible values. We'll use it to filter by a group of whitelisted addresses in the example below:
...
:::note
`OR` filtering on RPC data sources requires v3.3 or later. On earlier versions the array form was only honoured by HyperSync.
:::
...
- **[Envio Cloud](#envio-cloud)** — managed dashboards, alerts, and metrics for hosted indexers.
...
A slow indexer is usually waiting on one of two things: fetching events, or writing them. Since [`v3.5.0`](https://github.com/enviodev/hyperindex/releases/tag/v3.5.0) two counters attribute that wait directly, so you no longer have to infer it from handler timings:
...
```promql
# Fraction of the run spent stalled waiting on fetches
rate(envio_processing_stalled_on_fetch_seconds[5m])

# Share of the whole run so far, as a ratio between 0 and 1
envio_processing_stalled_on_fetch_seconds / envio_process_elapsed_seconds
```

The same trick works for any cumulative seconds metric — `envio_processing_seconds / envio_process_elapsed_seconds` gives the share of the run spent inside event handlers. When comparing two snapshots scraped manually rather than by Prometheus, `envio_process_metric_time_seconds` tells you when each was collected.
...
Use it to inspect indexing state and to manage the Effect API
...
If you deploy to Envio Cloud, observability is
...
See Monitoring Your Indexer and the
Envio Cloud features page for details,
and Envio Cloud CLI for monitoring deployments
...
A `getWhere` call on a field with no supporting index no longer runs unindexed forever — HyperIndex creates the index the first time a handler asks for it, then reuses it.
