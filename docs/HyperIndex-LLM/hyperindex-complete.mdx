...
A `getWhere` call on a field with no index creates one the first time a handler asks for it. That means `getWhere` works on **any** entity field, not just those marked `@index` or `@derivedFrom`.
