...
- Removed `rpc_config` in favor of `rpc`, which now supports multiple URLs, `for` mode (`sync`, `live`, `fallback`), and WebSocket configuration (see RPC for Live Indexing)
...
2. **If you were using loaders**, migrate them to Preload Optimization following the Migrating from Loaders guide.
