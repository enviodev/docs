...
:::note
Unordered mode is the default in V3 and recommended for most applications.
:::

The indexer processes events as soon as they're available from each chain, without waiting for other chains. This provides better performance and lower latency.
...
In V3 unordered is the default, so no flag is required. The V2 `unordered_multichain_mode` option and the `UNORDERED_MULTICHAIN_MODE` / `UNSTABLE__TEMP_UNORDERED_HEAD_MODE` environment variables have been removed.

### Ordered Mode

If your application requires strict deterministic ordering of events across all chains, you can opt into "Ordered Mode". In this mode, the indexer synchronizes event processing across all chains, ensuring that events are processed in the exact same order in every indexer run, regardless of which chain they came from.
...
- The indexer needs to wait for all blocks to increment from each chain
...
1. **Different Chain Speeds**: If one chain is significantly slower than others, consider using unordered mode to prevent bottlenecks.
