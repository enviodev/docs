...
The Uniswap V4 indexer showcases how to effectively structure a multichain indexer for a complex DeFi protocol. It handles high volumes of data across multiple networks while maintaining performance and reliability.
...
- Operations on your entities are [commutative](./terminology#commutative-property) — meaning the final result is the same regardless of which chain's events are processed first (e.g. incrementing a counter)
...
The primary trade-off is increased latency at the head of the chain. The indexer must wait for blocks from all chains to determine the correct ordering, so processing of recent events is delayed by the slowest chain's block time.

**Example:** If Chain A has 2-second blocks and Chain B has 15-second blocks, the indexer will process events at the slower 15-second rate to maintain proper ordering.

This latency is acceptable for applications where correct cross-chain ordering is more important than real-time updates. For bridge applications in particular, this ordering preservation can be critical for security and correctness — it ensures that deposit events on one chain are always processed before the corresponding withdrawal events on another chain.
