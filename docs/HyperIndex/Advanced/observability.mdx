...
exposes `/metrics`, `/metrics/runtime`, `/healthz`, and the `/console` API. Change
the port with `ENVIO_INDEXER_PORT`:
...
- The GraphQL (Hasura) endpoint and the Dev Console URL
...
- `progressBlock` — block number fully processed and written to the database.
- `eventsProcessed` — number of events processed and written to the database.
- `sourceBlock` — latest known block of the data source (the chain head).
- `isReady` / `readyAt` — whether the chain has finished its historical sync (and when).

See the [Metadata Query](/docs/HyperIndex/metadata-query) page for the full field
reference and update semantics.

## Dev Console

When you run `envio dev`, HyperIndex enables the **Dev Console** — a web UI for
debugging your indexer during local development. The TUI prints its URL on
startup:

```
https://envio.dev/console
```

The Dev Console is a hosted web app that connects to your locally running
indexer through the `/console` endpoint (default port `9898`), so your data never
...
All endpoints are served by the indexer on `ENVIO_INDEXER_PORT` (default `9898`).

| Endpoint | Description |
|----------|-------------|
| `/metrics` | Prometheus index metrics |
| `/metrics/runtime` | Node.js runtime metrics (CPU, memory, GC, event loop) |
| `/healthz` | Liveness probe — returns `200 OK` |
| `/console` | Dev Console API (development mode only) |
