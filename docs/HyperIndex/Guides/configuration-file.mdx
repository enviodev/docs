...
The `rpc` option configures an RPC data source per chain. It replaces the V2 `rpc_config` and supports multiple URLs with explicit roles via the `for` field:
...
For chains with HyperSync, RPC is used as a fallback by default for added reliability. For chains without HyperSync, RPC is the primary data source.
...
Set `end_block` on a chain to stop indexing at a specific block. Useful for backfills with a known cutoff, snapshots, or deterministic tests.
...
Set `block_lag` on a chain to keep the indexer a fixed number of blocks behind the chain head. This replaces the V2 `ENVIO_INDEXING_BLOCK_LAG` environment variable, applied per chain. Defaults to `0`.
...
Learn more about multichain indexing [here](./multichain-indexing).
...
Set `full_batch_size` to control how many events are processed in a single batch. This replaces the V2 `MAX_BATCH_SIZE` environment variable.
