...
Set `skip: true` on a chain to exclude it from indexing and database migrations without removing it from your config. This is handy for temporarily disabling a chain while keeping its configuration around.
  ...
  - id: 1
    ...
    contracts:
      - name: Greeter
        address: "0x9D02A17dE4E68545d3a58D3a20BbBE0399E05c9c"
  - id: 137
    skip: true # excluded from indexing and migrations
    start_block: 0
    contracts:
      - name: Greeter
        address: "0x9D02A17dE4E68545d3a58D3a20BbBE0399E05c9c"
...
:::note
The `skip` field was added in HyperIndex v3.1.
:::
...
Instead of routing every entity with an explicit `@storage` directive, you can mark a backend as `default` using the expanded object form. Entities without a `@storage` directive in `schema.graphql` are then automatically assigned to every backend marked `default: true`:
...
Use the `@storage` directive in `schema.graphql` only when you need to override this for a specific entity.
...
By default, database column names match the field names from your `schema.graphql`. Set `column_name_format: snake_case` on the Postgres backend to automatically convert column names to `snake_case` in the database, while keeping the original names in the GraphQL API and your handler types:
...
:::note
`column_name_format` was added in HyperIndex v3.2.
:::
