...
In V3, **unordered** multichain processing is the default — events from different chains are processed in parallel without strictly maintaining cross-chain event ordering. This minimizes indexing latency when indexing at the head.

If you need strict cross-chain ordering instead, opt in with:

```yaml
multichain: ordered
```

Learn more about multichain modes [here](./multichain-indexing).
