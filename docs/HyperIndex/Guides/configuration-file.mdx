...
For endpoints gated behind an auth header rather than a key in the URL, set `headers` on the RPC entry (added in v3.3). Values support `${ENV_VAR}` interpolation:
...
See [Custom HTTP Headers](/docs/HyperIndex/rpc-sync#custom-http-headers) for details.
...
Since v3.4, the `@storage` directive's `clickhouse` argument also accepts an options object for per-entity table tuning — partitioning, sorting key and TTL. See [Per-Entity ClickHouse Tuning](/docs/HyperIndex/schema#per-entity-clickhouse-tuning).
