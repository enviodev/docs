...
The top-level `contracts` block defines each contract once — its events, ABI, and per-event options. Each chain then references these contracts by `name` and supplies chain-specific values like the on-chain address (see [Contracts (per chain)](#contracts-per-chain)).

```yaml
contracts:
  - name: Greeter
    abi: ./abis/greeter.json
    events:
...
:::tip Automatic contract attachment
Contracts defined globally at the top level are automatically attached to every chain — you no longer need to list a global contract under each chain to make `addContract` / `context.chain.<Contract>.add` work. Only list the contract under a chain when you need to supply chain-specific values like an `address` or per-chain `start_block`.
:::

### ABI

You can either let Envio infer the ABI from the event signatures listed under `events`, or point to a JSON ABI file via `abi_file_path` (when you only want a subset of events from a larger ABI):
...
contracts:
  - name: Greeter
    ...
    events:
      - event: NewGreeting # referenced by name; signature comes from the ABI file
```
...
By default, all events listed here are indexed — remove an entry to stop indexing it.
