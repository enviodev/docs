...
2. Run the [`_meta` indexing status](/docs/HyperIndex/observability#indexing-status) query to see each chain's latest processed block and confirm the indexer is making progress
...
1. Run the [`_meta` indexing status](/docs/HyperIndex/observability#indexing-status) query to see each chain's latest processed block
