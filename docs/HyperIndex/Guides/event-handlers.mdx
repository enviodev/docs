...
You can filter by multiple fields at once by passing several conditions in the same object. They are combined with an `AND` relationship:
...
:::note
Multi-field filtering with `getWhere` was added in HyperIndex v3.2. Single `_eq` or `_in` filters are also optimized to reduce database round trips.
:::
