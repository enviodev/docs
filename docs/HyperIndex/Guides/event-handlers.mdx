...
You can access your indexer configuration within handlers via `indexer.chains[chainId]`:
...
indexer.onEvent(
  { contract: "Greeter", event: "NewGreeting" },
  async ({ event, context }) => {
    const chainConfig = indexer.chains[event.chainId];
  },
);
```
...
- `syncSource`, `startBlock`, `maxReorgDepth`
