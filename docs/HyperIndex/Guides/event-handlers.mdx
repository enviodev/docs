...
Below shows an example type of an event:
...
> Dev note: 📢 For indexers built using `ReScript`, use a lower case for the first letter of `entityName` when accessing it via context (i.e., `context.user` instead of `context.User`).
