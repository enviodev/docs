...
- Use [Topic Filtering](/docs/HyperIndex/wildcard-indexing) to ignore irrelevant events
  ...
  - With [filter by dynamically registered contract addresses](/docs/HyperIndex/wildcard-indexing#index-all-erc20-transfers-to-your-contract) (e.g. index all ERC20 transfers to/from your contract)
...
:::info Requirements
- [Preload Optimization](/docs/HyperIndex/preload-optimization) must be enabled (`preload_handlers: true` in `config.yaml`)
- The field must have an [`@index`](database-performance-optimization/#creating-custom-indices-in-your-schema) directive or be used in a [`@derivedFrom`](schema/#relationships-one-to-many-derivedfrom) relationship
:::

:::tip
Place `getWhere` calls at the top of your handler so they benefit from [Preload Optimization](/docs/HyperIndex/preload-optimization).
:::

:::warning
Very large `getWhere` result sets may cause memory issues. Filter as narrowly as possible.
:::
