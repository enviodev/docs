...
Starting from [`v3.4.0`](https://github.com/enviodev/hyperindex/releases/tag/v3.4.0), you can register as many handlers as you like for the same event, each with its own [`where`](/docs/HyperIndex/wildcard-indexing) filter. Earlier versions only accepted handlers whose filters matched.
...
- Works with any entity field. Since v3.5, if the field has no supporting database index, HyperIndex creates one the first time a handler queries it — you no longer need to declare [`@index`](database-performance-optimization/#creating-custom-indices-in-your-schema) up front. Declaring it is still worthwhile for fields you know you'll query: indexes named in the schema are built in one pass at the end of the backfill, whereas an on-demand build pauses writes to that table while it runs. See [Deferred Index Creation](/docs/HyperIndex/database-performance-optimization#deferred-index-creation).
...
Read more in the [Observability Guide](/docs/HyperIndex/observability).
