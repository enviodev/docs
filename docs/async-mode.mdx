...
With async-mode, [`loaders`](/docs/event-handlers#loader-function) become optional.
...
In the [testing framework](/docs/testing) itself, the only change is that `processEvent` is now `processEventAsync`, and it returns a promise rather than being synchronous.
