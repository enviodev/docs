...
- **Repository Folder**:
  ...
  - **Configuration file**: a HyperIndex [configuration file](/docs/v2/HyperIndex/configuration-file) must be present.

  The root folder and configuration file name can be set in the [indexer settings](#configure-your-indexer).
- **GitHub Repository**: The repository must be no larger than `100MB`. Caching between deployments is supported for paid plans using the [Effects Api](/docs/v2/HyperIndex/effect-api#cache-on-envio-cloud).
