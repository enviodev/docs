...
If you are new to HyperIndex, start with the [Getting Started](/docs/v2/HyperIndex/getting-started) guide first.
...
- **Up to 158x faster** historical sync via [HyperSync](/docs/v2/HyperIndex/hypersync)
...
> **v2 note**: HyperIndex v2 uses `networks` instead of `chains`. See the [v2→v3 migration guide](/docs/v2/HyperIndex/migrate-to-v3).
...
See the full list of available fields in the [Configuration File](/docs/v2/HyperIndex/config-schema-reference#fieldselection) docs.
...
**Indexes**: Replace `index().on(column)` with an [`@index`](/docs/v2/HyperIndex/schema) directive on the field.

**Relations**: Replace Ponder's `relations()` call with [`@derivedFrom`](/docs/v2/HyperIndex/schema) on the parent entity:
...
See the full [Schema](/docs/v2/HyperIndex/schema) docs.
...
See the [Event Handlers](/docs/v2/HyperIndex/event-handlers) docs for the full API reference.
...
Replace Ponder's `factory()` helper in config with a [`contractRegister`](/docs/v2/HyperIndex/dynamic-contracts) handler:
...
Replace `context.client.readContract(...)` with the [Effect API](/docs/v2/HyperIndex/effect-api) to safely isolate external calls from the sync path:
...
See [Multichain Indexing](/docs/v2/HyperIndex/multichain-indexing) for configuration details.

### Wildcard indexing

HyperIndex supports [wildcard indexing](/docs/v2/HyperIndex/wildcard-indexing) — index events by signature across all contracts on a chain without specifying addresses.
...
- **Docs**: [docs.envio.dev](/docs/v2/HyperIndex/overview)
